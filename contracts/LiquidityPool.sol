...
contract LiquidityPool is Ownable, TokenPool {
    ...
    /*function getTokensOut(address _token, uint _amount) 
        external 
        onlyOwner 
        returns (bool)
    {
        require(IERC20(_token).balanceOf(address(this)) >= _amount, "there is not enough funds");
        return IERC20(_token).transfer(msg.sender, _amount);
    }

    function withdraw() external onlyOwner returns (bool status) 
    {
        require(address(this).balance > 0, "not enough ether!");
        (status, ) = address(this).call{value: address(this).balance}("");
    }
